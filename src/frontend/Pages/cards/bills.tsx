...
import type { item, billRow, bill, purchase } from "@/src/global-types"
...
const BillRow = memo(({ item, onUpdate, onRemove, canRemove, t, isRTL, availableItems }: billRow) => {
    ...
    return (
        <TableRow className="border-b border-border select-none">
            ...
            <TableCell className="py-3 px-4">
                <Input
                    value={` ${item.total_price} ${t("currency")}`}
                    disabled
                    className="bg-muted border-border w-24 tabular-nums"
                />
            </TableCell>
    ...
    const getLatestBillId = useCallback(async () => {
        try {
            ...
            if (bills && bills.length > 0) {
                setBillId(bills[bills.length - 1].bill_id || 1)
            } else {
                setBillId(1)
            }
        } catch (error) {
